...
function processVideo() {
  ...
  setTimeout(processVideo, 30 / 1000);
}
...
if (customDensitySelector) {
  customDensitySelector.addEventListener('input', (e) => {
    selectedDensity =
      (e.target as HTMLSelectElement).value || defaultDensities[0];
...
if (densityOptions && densitySelector) {
  ...
  densitySelector.addEventListener('click', (e) => {
    densityOptions.style.display =
      densityOptions.style.display === 'block' ? 'none' : 'block';
  });
}
